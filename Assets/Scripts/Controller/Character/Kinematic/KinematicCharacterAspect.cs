...
namespace Controller.Character.Kinematic
{
    ...
    public readonly partial struct KinematicCharacterAspect : IAspect
    {
            ...
            where C : unmanaged
        {
            ...
            if (FilterColliderCastHitsForGroundProbing(in processor, ref context, ref baseContext, ref baseContext.TmpColliderCastHits, -characterBody.GroundingUp, characterData.ShouldIgnoreDynamicBodies(), out var closestHit))
            {
                ...
                {
                    var isGroundedOnClosestHit = processor.IsGroundedOnHit(ref context, ref baseContext, in groundHit, (int)GroundingEvaluationType.GroundProbing);
                        ...
                        if (baseContext.TmpColliderCastHits.Length > 1)
                        {
                            ...
                            foreach (var tmpHit in baseContext.TmpColliderCastHits)
                            {
                                ...
                                if (math.distancesq(tmpHitDistance, distanceToGround) <= Constants.GroundedHitDistanceToleranceSq)
                                {
                                    ...
                                    var isGroundedOnHit = processor.IsGroundedOnHit(ref context, ref baseContext, in tmpClosestGroundedHit, (int)GroundingEvaluationType.GroundProbing);
                                    if (isGroundedOnHit)
                                    {
                                        isGrounded = true;
                ...
                if (hitBodyEntity != characterBody.ParentEntity)
                {
                    ...
                    if (bodyHasPhysicsVelocityAndMass)
                    {
                        if (!baseContext.TmpRigidbodyIndexesProcessed.Contains(characterHit.RigidBodyIndex))
                        {
                            ...
                            var selfPhysicsVelocity = new PhysicsVelocity { Linear = characterBody.RelativeVelocity + characterBody.ParentVelocity, Angular = default };
                            ...
                            var otherIsCharacterMovingTowardsUs = otherIsCharacter && math.dot(otherPhysicsVelocity.Linear, effectiveHitNormalFromOtherToSelf) > Constants.DotProductSimilarityEpsilon;
            ...
            where C : unmanaged
        {
            ...
            {
                if (CalculateDistanceAllCollisions(in processor, ref context, ref baseContext, characterPosition, characterRotation, 0f, characterData.ShouldIgnoreDynamicBodies(), out var overlapHits))
                {
                    foreach (var overlapHit in overlapHits)
                    {
                        ...
                        if (math.dot(movementHit.Normal, characterBody.RelativeVelocity) < Constants.DotProductSimilarityEpsilon)
                        {
                            ...
                            var currentCharacterHit = KinematicCharacterUtilities.CreateCharacterHit(
                                in movementHit,
            ...
            if (characterData.DetectMovementCollisions)
            {
                ...
                while (movementCastIterationsMade < characterData.MaxContinuousCollisionsIterations && remainingMovementLength > 0f)
                {
                    ...
                    var castLength = remainingMovementLength + Constants.CollisionOffset; // TODO: shoud we keep this offset?

                    // Cast collider for movement
                    var castInput = new ColliderCastInput(characterPhysicsCollider.Value, castStartPosition, castStartPosition + (castDirection * castLength), characterRotation);
                    baseContext.TmpColliderCastHits.Clear();
                    var collector = new AllHitsCollector<ColliderCastHit>(1f, ref baseContext.TmpColliderCastHits);
                    baseContext.PhysicsWorld.CastCollider(castInput, ref collector);
                    var foundMovementHit = FilterColliderCastHitsForMove(in processor, ref context, ref baseContext, ref baseContext.TmpColliderCastHits, !characterData.SimulateDynamicBody, castDirection, Entity.Null, characterData.ShouldIgnoreDynamicBodies(), out var closestHit, out var foundAnyOverlaps);
            ...
            where C : unmanaged
        {
            ...
            while (decollisionIterationsMade < characterData.MaxOverlapDecollisionIterations)
            {
                ...
                var distanceInput = new ColliderDistanceInput(characterPhysicsCollider.Value, 0f, math.RigidTransform(characterRotation, characterPosition));
                ...
                FilterDistanceHitsForSolveOverlaps(in processor, ref context, ref baseContext, ref baseContext.TmpDistanceHits, out var mostPenetratingHit, out var mostPenetratingDynamicHit, out var mostPenetratingNonDynamicHit);
                ...
                if (characterData.SimulateDynamicBody)
                {
                    ...
                    foreach (var dynamicHit in baseContext.TmpDistanceHits)
                    {
                        ...
                        if (characterData.EvaluateGrounding)
                        {
                            isGroundedOnHit = processor.IsGroundedOnHit(ref context, 
                                ref baseContext,
                                in basicDynamicHit, 
                ...
                else
                {
                    ...
                    if (isLastIteration)
                    {
                        foreach (var dynamicHit in baseContext.TmpDistanceHits)
                        {
                            ...
                            var ovelapHit = KinematicCharacterUtilities.CreateCharacterHit(
                                in basicDynamicHit,
                                characterBody.IsGrounded,
                                characterBody.RelativeVelocity,
                                false);
            ...
            where C : unmanaged
        {
            ...
            {
                var ovelapCharacterHit = KinematicCharacterUtilities.CreateCharacterHit(
                    in hit,
                    characterBody.IsGrounded,
                ...
                math.lengthsq(relativeVelocity) > Constants.MinVelocityLengthSqForGroundingIgnoreCheck)
            {
                if (PhysicsUtilities.DoesBodyHavePhysicsVelocityAndMass(in physicsWorld, hit.RigidBodyIndex))
                {
                    PhysicsUtilities.GetBodyComponents(
                        in physicsWorld,
                        hit.RigidBodyIndex,
                        out var hitTransform,
                        out var hitPhysicsVelocity,
                        out var hitPhysicsMass);
            ...
            where C : unmanaged
        {
            ...
            if (maxStepHeight > 0f)
            {
                ...
                // Close back step hit
                var backStepHitFound = RaycastClosestCollisions(
                    in processor,
                    ref context,
                    ref baseContext,
                    ...
                    out var backStepHit,
                    out var backHitDistance);
                if (backStepHitFound && backHitDistance > 0f)
                    isGroundedOnBackStep = IsGroundedOnSlopeNormal(characterData.MaxGroundedSlopeDotProduct, backStepHit.SurfaceNormal, characterBody.GroundingUp);
                ...
                if (isGroundedOnBackStep)
                {
                    ...
                    var forwardStepHitFound = RaycastClosestCollisions(
                        in processor,
                        ref context,
                        ref baseContext,
                        ...
                        characterData.ShouldIgnoreDynamicBodies(),
                        out var forwardStepHit,
                        out var forwardHitDistance);
                    if (forwardStepHitFound && forwardHitDistance > 0f)
                    {
                        isGroundedOnForwardStep = IsGroundedOnSlopeNormal(characterData.MaxGroundedSlopeDotProduct, forwardStepHit.SurfaceNormal, characterBody.GroundingUp);
                    }
            ...
            where C : unmanaged
        {
                ...
                !PhysicsUtilities.IsBodyDynamic(in baseContext.PhysicsWorld, hit.RigidBodyIndex))
            {
                ...
                // Up cast
                var foundUpStepHit = CastColliderClosestCollisions(
                    in processor,
                    ref context,
                    ref baseContext,
                    ...
                    characterData.ShouldIgnoreDynamicBodies(),
                    out var upStepHit,
                    out var upStepHitDistance);

                upStepHitDistance = foundUpStepHit
                    ? math.max(0f, upStepHitDistance - Constants.CollisionOffset)
                    : upCheckDistance;
                ...
                if (upStepHitDistance > 0f)
                {
                    ...
                    var endPositionOfForwardCheck = startPositionOfForwardCheck + (remainingMovementDirection * (remainingMovementLength + Constants.CollisionOffset));
                    var minimumDistanceOverStep = Constants.CollisionOffset * 3f;
                    ...
                    // Forward cast
                    var foundForwardStepHit = CastColliderClosestCollisions(
                        in processor,
                        ref context,
                        ref baseContext,
                        ...
                        characterData.ShouldIgnoreDynamicBodies(),
                        out var forwardStepHit,
                        out var forwardStepHitDistance);

                    forwardStepHitDistance = foundForwardStepHit
                        ? math.max(0f, forwardStepHitDistance - Constants.CollisionOffset)
                        : forwardCheckDistance;
                    ...
                    if (forwardStepHitDistance > 0f)
                    {
                        ...
                        // Down cast
                        var foundDownStepHit = CastColliderClosestCollisions(
                            in processor,
                            ref context,
                            ref baseContext, 
                            ...
                            {  
                                ...
                                if(characterWidthForStepGroundingCheck > 0f)
                                {
                                    ...
                                    if (RaycastClosestCollisions(
                                            ...
                                            maxStepHeight,
                                            characterData.ShouldIgnoreDynamicBodies(),
                                            out var forwardSlopeCheckHit,
                                            out var forwardSlopeCheckHitDistance))
                                    {
            ...
            where C : unmanaged
        {
            ...
            if (IsGroundedOnSlopeNormal(characterData.MaxGroundedSlopeDotProduct, characterBody.GroundHit.Normal, characterBody.GroundingUp))
            {
                ...
                {
                    // Raycast forward 
                    var forwardHitFound = RaycastClosestCollisions(
                        in processor,
                        ref context,
                        ref baseContext,
                        rayStartPoint,
                    ...
                    else
                    {
                        ...
                        // Raycast down 
                        var downHitFound = RaycastClosestCollisions(
                            in processor,
                            ref context,
                            ref baseContext,
                            rayStartPoint,
                        ...
                        if (downHitFound)
                        {
                            ...
                            if (!IsGroundedOnSlopeNormal(characterData.MaxGroundedSlopeDotProduct, downHit.SurfaceNormal, characterBody.GroundingUp))
                            {
                        ...
                        if(isMovingTowardsNoGrounding)
                        {
                            ...
                            var secondDownHitFound = RaycastClosestCollisions(
                                in processor,
                                ref context,
                                ref baseContext, 
                                rayStartPoint,
                                ...
                                rayLength,
                                characterData.ShouldIgnoreDynamicBodies(),
                                out var secondDownHit,
                                out var secondDownHitDistance);
                            ...
                            if (secondDownHitFound)
                            {
                                ...
                                if (IsGroundedOnSlopeNormal(characterData.MaxGroundedSlopeDotProduct, secondDownHit.SurfaceNormal, characterBody.GroundingUp))
                                    isMovingTowardsNoGrounding = false;
                            }
                            else
                            {
                                ...
                                // Raycast backward
                                var backHitFound = RaycastClosestCollisions(
                                    in processor,
                                    ref context,
                                    ref baseContext,
                                    rayStartPoint,
                                ...
                                if (backHitFound)
                                {
                                    ...
                                    if (IsGroundedOnSlopeNormal(characterData.MaxGroundedSlopeDotProduct, backHit.SurfaceNormal, characterBody.GroundingUp))
                                        isMovingTowardsNoGrounding = false;
                                }
                            }
                        }
        ...
        public bool IsGroundedOnSlopeNormal(float maxGroundedSlopeDotProduct, float3 slopeSurfaceNormal, float3 groundingUp)
        {
