...
namespace Controller.Character.Kinematic
{
    public readonly partial struct KinematicCharacterAspect
    {
            ...
            where C : unmanaged
        {
            ...
            var castInput = new ColliderCastInput(characterPhysicsCollider.Value, characterPosition, characterPosition + (direction * length), characterRotation);
            ...
            if (FilterColliderCastHitsForClosestCollisions(in processor, ref context, ref baseContext,
                    ref baseContext.TmpColliderCastHits, onlyObstructingHits, direction, ignoreDynamicBodies, out var closestHit))
            {
                hit = closestHit;
