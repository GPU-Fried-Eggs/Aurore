...
namespace Character.Kinematic
{
    public readonly partial struct KinematicCharacterAspect
    {
        ...
        public bool DefaultIsGroundedOnHit<T, C>(in T processor,
            ref C context,
            ...
            where C : unmanaged
        {
            ...
            {
                var hitIsOnCharacterBottom = math.dot(characterBody.GroundingUp, hit.Normal) > k_DotProductSimilarityEpsilon;
                if (hitIsOnCharacterBottom && groundingEvaluationType is (int)GroundingEvaluationType.GroundProbing or (int)GroundingEvaluationType.StepUpHit)
                {
                    ...
                    {
                        isGroundedOnSteps = IsGroundedOnSteps(in processor, ref context, ref baseContext, in hit,
                            stepAndSlopeHandling.MaxStepHeight, stepAndSlopeHandling.ExtraStepChecksDistance);
                    }
                }
            }
        ...
        public void DefaultProjectVelocityOnHits(ref float3 velocity,
        ...
        {
            bool IsSamePlane(float3 planeA, float3 planeB) => math.dot(planeA, planeB) > (1f - k_DotProductSimilarityEpsilon);

            void ProjectVelocityOnSingleHit(ref float3 velocity,
                ref bool characterIsGrounded,
                ref BasicHit characterGroundHit,
                in KinematicVelocityProjectionHit hit,
                float3 groundingUp)
            {
                ...
                if (hit.IsGroundedOnHit || !constrainToGoundPlane)
                {
                    ...
                    if (hit.RigidBodyIndex >= 0)
                    {
                        ...
                        if (math.dot(groundingUp, hit.Normal) > k_DotProductSimilarityEpsilon)
                        {
            ...
            if (math.dot(velocityDirection, firstHit.Normal) < 0f)
            {
                ...
                for (var secondHitIndex = -1; secondHitIndex < hitsCount; secondHitIndex++)
                {
                    ...
                    if (math.dot(velocityDirection, secondHit.Normal) > -k_DotProductSimilarityEpsilon)
                        continue;
                    ...
                    if (math.dot(velocityDirection, firstHit.Normal) > -k_DotProductSimilarityEpsilon)
                        continue;
                    ...
                    for (var thirdHitIndex = -1; thirdHitIndex < hitsCount; thirdHitIndex++)
                    {
                        ...
                        if (math.dot(velocityDirection, thirdHit.Normal) < -k_DotProductSimilarityEpsilon)
                        {
            ...
            where C : unmanaged
        {
            ...
            if (stepHandling && !hit.IsGroundedOnHit &&
                math.dot(math.normalizesafe(characterBody.RelativeVelocity), characterBody.GroundingUp) > k_MinVelocityDotRatioWithGroundingUpForSteppingUpHits)
            {
                CheckForSteppingUpHit(in processor,
                    ref context,
                    ref baseContext,
                    ref characterBody,
