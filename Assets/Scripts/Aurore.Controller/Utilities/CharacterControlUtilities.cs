...
namespace Utilities
{
    public static class CharacterControlUtilities
    {
        ...
        {
            var moveDirectionOnSlopePlane = math.normalizesafe(MathUtilities.ProjectOnPlane(moveDirection, slopeNormal));
            var angleRadiansWithUp = MathUtilities.AngleRadians(moveDirectionOnSlopePlane, groundingUp);

            return useDegrees ? 90f - math.degrees(angleRadiansWithUp) : math.PI * 0.5f - angleRadiansWithUp;
        }
        ...
        public static void StandardGroundMoveInterpolated(ref float3 velocity, float3 targetVelocity, float sharpness, float deltaTime, float3 groundingUp, float3 groundedHitNormal)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void StandardGroundMoveAccelerated(ref float3 velocity, float3 acceleration, float maxSpeed, float deltaTime, float3 groundingUp, float3 groundedHitNormal, bool forceNoMaxSpeedExcess)
        {
            var addedVelocityFromAcceleration = float3.zero;
        ...
        public static void ClampAdditiveVelocityToMaxSpeedOnPlane(ref float3 additiveVelocity, float3 originalVelocity, float maxSpeed, float3 movementPlaneUp, bool forceNoMaxSpeedExcess)
        {
            ...
            {
                var totalVelocity = originalVelocity + additiveVelocity;
                var velocityUp = math.projectsafe(totalVelocity, movementPlaneUp);
                var velocityHorizontal = MathUtilities.ProjectOnPlane(totalVelocity, movementPlaneUp);
            ...
            else
            {
                ...
                var additiveVelocityOnPlaneUp = math.projectsafe(additiveVelocity, movementPlaneUp);
                var additiveVelocityOnPlane = additiveVelocity - additiveVelocityOnPlaneUp;

                var originalVelocityOnPlaneUp = math.projectsafe(originalVelocity, movementPlaneUp);
                var originalVelocityOnPlane = originalVelocity - originalVelocityOnPlaneUp;

                var totalVelocityOnPlane = originalVelocityOnPlane + additiveVelocityOnPlane;

                if (math.lengthsq(totalVelocityOnPlane) > maxSpeedSq)
                {
                    var originalVelocityForwardOnPlane = math.normalizesafe(originalVelocityOnPlane);
                    var totalVelocityDirectionOnPlane = math.normalizesafe(totalVelocityOnPlane);

                    var totalClampedVelocityOnPlane = float3.zero;
                    if (math.dot(totalVelocityDirectionOnPlane, originalVelocityForwardOnPlane) > 0f)
                    {
                        var originalVelocityRightOnPlane = math.normalizesafe(math.cross(originalVelocityForwardOnPlane, movementPlaneUp));

                        // trim additive velocity excess in original velocity direction
                        var trimmedTotalVelocityForwardComponent = MathUtilities.ClampToMaxLength(math.projectsafe(totalVelocityOnPlane, originalVelocityForwardOnPlane), math.max(maxSpeed, math.length(originalVelocityOnPlane)));
                        var trimmedTotalVelocityRightComponent = MathUtilities.ClampToMaxLength(math.projectsafe(totalVelocityOnPlane, originalVelocityRightOnPlane), maxSpeed);
                    ...
                    var clampedAdditiveVelocityOnPlane = totalClampedVelocityOnPlane - originalVelocityOnPlane;
        ...
        public static void StandardJump(ref KinematicCharacterBody characterBody, float3 jumpVelocity, bool cancelVelocityBeforeJump, float3 velocityCancelingUpDirection)
        {
            ...
            if (cancelVelocityBeforeJump)
                characterBody.RelativeVelocity =
                    MathUtilities.ProjectOnPlane(characterBody.RelativeVelocity, velocityCancelingUpDirection);
        ...
        {
            return deltaTime > 0f ? positionDelta / deltaTime : default;
        }
        ...
        {
            var targetRotation = MathUtilities.CreateRotationWithUpPriority(direction, MathUtilities.GetForwardFromRotation(rotation));
