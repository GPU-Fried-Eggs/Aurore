...
namespace Character.Kinematic
{
    ...
    public struct AuthoringKinematicCharacterData
    {
        ...
        [Tooltip("Enables interpolating the character's rotation between fixed update steps, for smoother movement. " +
                 "Note: the Standard Characters must keep this option disabled, because by default they all handle " +
                 "their rotation on regular update rather than fixed update")]
        ...
        [Tooltip("Computes a more precise distance to ground hits when the original query returned a distance of 0f " +
                 "due to imprecisions. Helps reduce jitter in certain situations, but can have an additional " +
                 "performance cost. It is recommended that you only enable this if you notice jitter problems when " +
                 "moving against angled walls")]
        ...
        [Tooltip("Enables doing an extra physics check to project velocity on initial overlaps before the character " +
                 "moves. This can help with tunneling issues when you rotate your character in a way that could " +
                 "change the detected collisions (which doesn\'t happen if your character has an upright capsule " +
                 "shape and only rotates around up axis, for example), but it has a performance cost.")]
        ...
        [Header("Dynamics")]
        [Tooltip("Enables physics interactions (push and be pushed) with other dynamic bodies. Note that in order to " +
                 "be pushed properly, the character's collision response has to be either \"None\" or \"Raise Trigger Events\"")]
    ...
    public struct KinematicCharacterData : IComponentData
    {
        ...
        public float GroundSnappingDistance;
        /// <summary>
        /// Computes a more precise distance to ground hits when the original query returned a distance of 0f due to
        /// imprecisions. Helps reduce jitter in certain situations, but can have an additional performance cost.
        /// It is recommended that you only enable this if you notice jitter problems when moving against angled walls
        /// </summary>
        public bool EnhancedGroundPrecision;
        /// <summary>
        ...
        public bool DecollideFromOverlaps;
        /// <summary>
        /// Enables doing an extra physics check to project velocity on initial overlaps before the character moves.
        /// This can help with tunneling issues when you rotate your character in a way that could change the detected
        /// collisions (which doesn't happen if your character has an upright capsule shape and only rotates around up
        /// axis, for example), but it has a performance cost.
        /// </summary>
        public bool ProjectVelocityOnInitialOverlaps;
        /// <summary>
