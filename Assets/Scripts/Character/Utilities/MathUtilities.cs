...
namespace Character.Utilities
{
    public static class MathUtilities
    {
        ...
        public static quaternion FromToRotation(quaternion from, quaternion to) => math.mul(math.inverse(from), to);
        ...
        public static float AngleRadians(float3 from, float3 to)
        {
            ...
            if (denominator < math.EPSILON) return 0F;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float AngleRadiansToDotRatio(float angleRadians) => math.cos(angleRadians);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float DotRatioToAngleRadians(float dotRatio) => math.acos(dotRatio);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 ProjectOnPlane(float3 vector, float3 onPlaneNormal) => vector - math.projectsafe(vector, onPlaneNormal);
        ...
        {
            var projectionRatio = math.dot(math.normalizesafe(projectedVector), onNormalizedVector);
            if (projectionRatio == 0f) return projectedVector;

            var deprojectedLength = math.clamp(math.length(projectedVector) / projectionRatio, 0f, maxLength);
        ...
        {
            var sqrMag = math.lengthsq(vector);
            if (sqrMag > maxLength * maxLength)
            {
                ...
                return new float3(normalizedX * maxLength, normalizedY * maxLength, normalizedZ * maxLength);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 GetUpFromRotation(quaternion rot) => math.mul(rot, math.up());
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 GetRightFromRotation(quaternion rot) => math.mul(rot, math.right());
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 GetForwardFromRotation(quaternion rot) => math.mul(rot, math.forward());
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float GetSharpnessInterpolant(float sharpness, float dt) => math.saturate(1f - math.exp(-sharpness * dt));
        ...
        public static float3 ReorientVectorOnPlaneAlongDirection(float3 vector, float3 onPlaneNormal, float3 alongDirection)
        {
            ...
            if (length <= math.EPSILON) return float3.zero;

            var reorientAxis = math.cross(vector, alongDirection);
            var reorientedVector = math.normalizesafe(math.cross(onPlaneNormal, reorientAxis)) * length;
        ...
        {
            var pointLocalPositionRelativeToPreviousTransform = math.transform(math.inverse(fromTransform), pointWorldSpace);
            var pointNewWorldPosition = math.transform(toTransform, pointLocalPositionRelativeToPreviousTransform);
        ...
        {
            var targetBodyRigidTransform = new RigidTransform
            {
                pos = bodyRigidTransform.pos + (linearVelocity * deltaTime),
                rot = math.mul(bodyRigidTransform.rot, quaternion.Euler(angularVelocity * deltaTime))
            };
        ...
        {
            var localPointToTranslation = math.mul(math.inverse(rotation), position - aroundPoint);
